...
/**
 * Load SNP data from a simple space-separated file.
 ...
 *
 * In order to load the SNP files specify the following properties:
 * <dl>
 * <dt><code>load.inputDirectory</code></dt><dd>The path of the directory that contains valid SNP files, e.g. "/Users/rn2/variationDB" (Required)</dd>
 * <dt><code>load.fileNamePattern</code></dt><dd>The name of the file / pattern (default is *.snp) eg: ssaha.snp</dd>
 *
 * @author rn2
 *
 */
public class LoadSNPs extends FileProcessor {
    ...
    private final SNPsLoader loader;
    private LoadSNPs() {
        ApplicationContext applicationContext = new ClassPathXmlApplicationContext(new String[] {"Load.xml"});
        this.loader = applicationContext.getBean("snpsLoader", SNPsLoader.class);
    }
...
class SNPFile {
    ...
    // ANALYSIS_NAME=MAQ##ANALYSIS_PARAMS= -blah -blah#ANALYSIS_TIMESTAMP=1010101010##FIELD_SEPERATOR="\t"
    class MetaDataLine {
        int numberOfFields;
    ...
    public SNPFile(File file, Reader reader) throws IOException, ParsingException {
        this.file = file;
...
class SNPsLoader {
    ...
    public void load(SNPFile snpFile,String dbSchema) throws DataError {
        ...
        try{
        if(snpFile.metaline().getAnalysisTimestamp().length()==0){
        analysistimestamp = new Date();
        }
        else{
        analysistimestamp  = df.parse(snpFile.metaline().getAnalysisTimestamp());
        }
        }catch(java.text.ParseException e){
            String errorMessage = String.format("ANALYSIS_TIMESTAMP %s not in proper format yyyy-MM-dd-HH-mm-ss",snpFile.metaline().getAnalysisTimestamp());
        ...
        int n=simpleJdbcTemplate.update(
            ...
            "?,?,?"+
            ")",
            snpFile.metaline().getAnalysisName(), snpFile.metaline().getAnalysisProgramVersion(),analysistimestamp);
        if (n != 1) {
            throw new DataError(String.format("Unable to insert Analysis '%s'", snpFile.metaline().getAnalysisName()));
        }
        ...
        Object[] commandlineprop = new Object[] {analysisId,"commandline_str",cvId,snpFile.metaline().getAnalysisParams()};
        batch.add(commandlineprop);
        Object[] strain_id_prop = new Object[] {analysisId,"strain_id",cvId,snpFile.metaline().get_Strain_ID()};
        batch.add(strain_id_prop);
        Object[] strain_sample_id_prop = new Object[] {analysisId,"strain_sample_id",cvId,snpFile.metaline().get_Strain_Sample_ID()};
        ...
        SNPFile.Line headers = snpFile.headerline();
        ...
        // process data
        // each line is a SNP so create a feature record for every line
        // each field within a line is featureprop of the SNP (feature)

        for (SNPFile.Line line: snpFile.datalines()) {
            ...
            n = simpleJdbcTemplate.update(
                ...
                ")"
                ,snpFile.metaline().get_Chado_Organism_ID(),snpFeatureUniqueName,new java.sql.Timestamp(analysistimestamp.getTime()),new java.sql.Timestamp(analysistimestamp.getTime()));
            if (n != 1) {
                throw new DataError(String.format("Unable to insert SNP '%s'",snpFeatureUniqueName));
            }
            ...
            if (n != 1) {
                throw new DataError(String.format("Unable to insert featureLoc for SNP '%s'",snpFeatureUniqueName));
            }
            ...
            for ( String datafield : line.getFields()){
                ...
                if (n != 1) {
                    throw new DataError(String.format("Unable to insert featureprop for SNP '%s'",snpFeatureUniqueName));
                }
         ...
         // now associate this feature with the Analysis using AnalysisFeature record
            ...
            if (n != 1) {
                throw new DataError(String.format("Unable to insert record into AnalysisFeature for SNP '%s'",snpFeatureUniqueName));
            }
